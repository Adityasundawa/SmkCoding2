...
class MyUpdateActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        getData(); //Mengambil dari intent data yang akan di update
        update.setOnClickListener {
            ...
            if (isEmpty(cekNama) || isEmpty(cekEmail) || isEmpty(cekTelp) ||
                ...
                val temanBaru = Konsul(cekNama!!, cekEmail!!, cekTelp!!,
                    cekAlamat!!)
                val getUserID: String =
                    auth?.getCurrentUser()?.getUid().toString()
                val getKey: String =
                    getIntent().getStringExtra("getPrimaryKey").toString()
    ...
    private fun getData() {
        ...
        val getEmail: String =
            getIntent().getExtras()?.getString("dataEmail").toString()
        val getTelp: String =
            getIntent().getExtras()?.getString("dataTelp").toString()
        val getAlamat: String =
            getIntent().getExtras()?.getString("dataAlamat").toString()
